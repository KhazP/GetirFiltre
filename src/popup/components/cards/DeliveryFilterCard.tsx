...
export default function DeliveryFilterCard({ settings, updateSetting, disabled, variant = 'default' }: CardProps) {
    const content = (
    ...
    if (variant === 'compact') {
        return (
            <div className="bg-gf-dark-800 rounded-lg p-4 transition-colors hover:bg-gf-dark-750">
                <label className="block text-sm font-medium text-gray-300 mb-2">Maksimum Teslimat Süresi</label>
                {content}
                <p className="text-xs text-gray-500 mt-1">Bu süreden uzun teslimat süreleri gizlenir</p>
            </div>
        );
    }
